...
const LoginForm = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Ensure the URL matches your Eclipse Project Name and Servlet Mapping
      ...
      if (response.data.status === 'success') {
        // Redirect to CRUD Application upon valid credentials 
        navigate('/dashboard'); 
      }
    } catch (err) {
      // Display mandatory 'Invalid User' message 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#e0f2ff] via-[#f0f9ff] to-white flex flex-col items-center px-6 py-12">
      <div className="self-start mb-4">
        <h1 className="text-[#1734a1] text-2xl font-black italic tracking-tighter cursor-pointer" onClick={() => navigate('/')}>TALENTLINK</h1>
      </div>
