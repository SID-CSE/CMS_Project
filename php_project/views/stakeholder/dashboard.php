...
$activePath = '/stakeholder/dashboard';
$quickLinks = [
    ['label' => 'Create Request', 'href' => '/stakeholder/create-project-request', 'description' => 'Submit a fresh project request.'],
    ['label' => 'Projects', 'href' => '/stakeholder/projects', 'description' => 'Review progress and approvals.'],
    ['label' => 'Messages', 'href' => '/stakeholder/messages', 'description' => 'Follow up with the team.'],
    ['label' => 'Notifications', 'href' => '/stakeholder/notifications', 'description' => 'Check the latest updates.'],
];
?>
<?php require_once __DIR__ . '/../partials/role-topbar.php'; ?>
<div class="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-100 via-white to-emerald-50 text-slate-900">
    <div class="absolute left-[-6rem] top-[-6rem] h-56 w-56 rounded-full bg-emerald-300/20 blur-3xl"></div>
    ...
    <?php require_once __DIR__ . '/../partials/role-sidebar.php'; ?>
            ...
            <section class="rounded-[28px] border border-slate-200/80 bg-white/90 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur">
                <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                    <div class="max-w-2xl">
                        ...
                        <h2 class="mt-2 text-4xl font-black tracking-tight text-slate-950">Stakeholder Dashboard</h2>
                        <p class="mt-3 max-w-xl text-sm leading-7 text-slate-500">Track your requests, review progress, and stay on top of messages and approvals.</p>
                        <div class="mt-5 flex flex-wrap gap-3">
                            <a href="/stakeholder/projects" class="rounded-xl bg-[#1734a1] px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 hover:bg-blue-800">Open Projects</a>
                            <a href="/stakeholder/messages" class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">Open Messages<?php if (($unreadThreads ?? 0) > 0): ?> (<?= (int) $unreadThreads ?>)<?php endif; ?></a>
                            <a href="/stakeholder/profile" class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">Profile</a>
                        </div>
                    </div>
            ...
            <section class="mt-6 grid gap-4 md:grid-cols-4">
                <div class="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Total Requests</p>
                    <p class="mt-3 text-4xl font-black tracking-tight text-slate-950"><?= (int) ($stats['total'] ?? 0) ?></p>
                </div>
                <div class="rounded-[24px] border border-blue-100 bg-white p-5 shadow-sm">
                    ...
                    <p class="mt-3 text-4xl font-black tracking-tight text-blue-700"><?= (int) ($stats['requested'] ?? 0) ?></p>
                </div>
                <div class="rounded-[24px] border border-purple-100 bg-white p-5 shadow-sm">
                    ...
                    <p class="mt-3 text-4xl font-black tracking-tight text-purple-700"><?= (int) ($stats['inProgress'] ?? 0) ?></p>
                </div>
                <div class="rounded-[24px] border border-emerald-100 bg-white p-5 shadow-sm">
                    ...
                    <p class="mt-3 text-4xl font-black tracking-tight text-emerald-700"><?= (int) ($stats['signedOff'] ?? 0) ?></p>
                </div>
            </section>
            ...
            <section class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.8fr)]">
                <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="mb-5 flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-slate-500">Latest project requests and status updates</p>
                        </div>
                        <a href="/stakeholder/projects" class="text-sm font-semibold text-blue-700 hover:underline">View all</a>
                    </div>
                ...
                <aside class="space-y-6">
                    <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-4 space-y-3">
                            <?php foreach ($quickLinks as $link): ?>
                                <a href="<?= htmlspecialchars($link['href'], ENT_QUOTES, 'UTF-8') ?>" class="block rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 transition hover:border-blue-200 hover:bg-emerald-50">
                    ...
                    <div class="rounded-[28px] border border-slate-200 bg-slate-950 p-6 text-white shadow-[0_18px_50px_rgba(15,23,42,0.18)]">
                        <div class="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Review pulse</div>
                        <p class="mt-3 text-2xl font-bold tracking-tight">Approvals are visible</p>
                        <p class="mt-3 text-sm leading-7 text-slate-300">One place to monitor requests, feedback, and sign-off status as work moves forward.</p>
                    </div>
                </aside>
            </section>
            </div>
